...
export class Text_displayer {
  ...
  private display_text(
    text: string,
    point: Point,
    options: Required<TextDisplayOptions>
  ) {
    // determine color
    function get_color(
      colors: Array<Color>,
      ...
      if (index >= colors.length) return color;
      return colors[index];
    }
    ...
    const write_text = (x: number, y: number) => {
      ...
      for (let i = 0; i < text.length; i++) {
        this.write_char(
          text[i],
          { x: x_cursor, y: y },
          get_color(options.colors, options.color, i)
        );
  ...
  public display(text: string, options?: TextDisplayOptions) {
    ...
    const filled_opt: Required<TextDisplayOptions> = {
      ...defaultOptions,
      ...options,
    };
    ...
    if (text_width <= this.screen_.size_get().x) {
      // the text is smaller than the screen
